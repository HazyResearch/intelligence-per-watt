#!/usr/bin/env python3
"""Build and stage the Rust energy monitor binary for the Python package."""
...
ENERGY_MONITOR_ROOT = PROJECT_ROOT.parent / "energy-monitor"
BIN_ROOT = PROJECT_ROOT / "src" / "ipw" / "telemetry" / "bin"

# Mapping from sys.platform / machine combos to folder names under bin/
PLATFORM_ALIASES = {
...
def copy_binary(
    ...
    if not is_windows:
        dest.chmod(0o755)
    print(f"Copied {src.relative_to(PROJECT_ROOT.parent)} -> {dest.relative_to(PROJECT_ROOT)}")


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build and stage the energy monitor binary for the Python package."
    )
    parser.add_argument(
        "--target",
    ...
    if not ENERGY_MONITOR_ROOT.exists():
        raise SystemExit(
            f"Energy monitor source not found at {ENERGY_MONITOR_ROOT}.\n"
            f"Expected sibling directory: ../energy-monitor/"
        )
